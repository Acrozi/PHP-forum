...
if(!empty($_POST["login"]) && !empty($_POST["email"]) && !empty($_POST["password"])) {
	...
	if($owner->login()) {
		...
		$loginMessage = 'Ogiltig inloggning! Försök igen.';
	}
} else if (!isset($_POST["login"])){
	$loginMessage = 'Ange email och lösenord för att logga in.';
}

include('inc/header.php');
?>
<title>PHP AJAX Forum</title>
			...
			<div style="padding-top:30px;width:400px;" class="panel-body" >
				...
				<form id="loginform" class="form-horizontal" role="form" method="POST" action="">
					...
					<div style="margin-bottom: 25px" class="input-group">
						...
						<input type="password" class="form-control" id="password" name="password" value="<?php if(!empty($_POST["password"])) { echo $_POST["password"]; } ?>" placeholder="lösenord" required>
					</div>
